...
- [ ] **`AppSelect` component** — replace every raw `<select>` in the codebase (status filter, sort order, city dropdown, bulk status, rejection reason, etc.) with a shared `src/components/ui/AppSelect.vue`. Internally still a native `<select>` but with `appearance: none`, a custom SVG chevron icon, `border-radius: 0.5rem`, `border: 1px solid var(--col-border)`, `padding: 0.5rem 2.25rem 0.5rem 0.75rem`, and focus ring matching other inputs. The native `<option>` list is OS-rendered (unavoidable cross-browser) but the closed state will match everything else. Props: `modelValue`, `options: { value, label }[]`, `placeholder`. Emits `update:modelValue`.
